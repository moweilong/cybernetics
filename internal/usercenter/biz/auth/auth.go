...
//go:generate mockgen -self_package github.com/moweilong/cybernetics/internal/usercenter/biz/auth -destination mock_auth.go -package auth github.com/moweilong/cybernetics/internal/usercenter/biz/auth AuthBiz
...
func (b *authBiz) Logout(ctx context.Context, rq *v1.LogoutRequest) error {
	if err := b.authn.Destroy(ctx, cyberneticsx.FromAccessToken(ctx)); err != nil {
...
func (b *authBiz) RefreshToken(ctx context.Context, rq *v1.RefreshTokenRequest) (*v1.LoginReply, error) {
	...
	_ = b.authn.Destroy(ctx, cyberneticsx.FromAccessToken(ctx))

	userID := cyberneticsx.FromUserID(ctx)
