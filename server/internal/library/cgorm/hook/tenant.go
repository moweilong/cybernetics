...
type hookSaveTenant struct {
	...
	in          any
	isNewRecord bool
	relations   map[int64]*cgorm.TenantRelation
}
...
func newHookSaveTenant(ctx context.Context, in any) (*hookSaveTenant, error) {
	...
	h.relations = make(map[int64]*cgorm.TenantRelation)
	return h, nil
}
...
func (h *hookSaveTenant) getRelation(id int64) (tr *cgorm.TenantRelation, err error) {
	v, ok := h.relations[id]
	if ok {
		return v, nil
	}

	h.relations[id], err = cgorm.GetTenantRelation(h.ctx, id)
	if err != nil {
		return nil, err
	}
