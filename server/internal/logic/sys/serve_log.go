...
func (s *sSysServeLog) List(ctx context.Context, in *sysin.ServeLogListInp) (list []*sysin.ServeLogListModel, totalCount int, err error) {
	...
	// 关联表sysLog
	mod = mod.LeftJoin(cgorm.GenJoinOnRelation(
	...
	// 关联表select
	fields, err := cgorm.GenJoinSelect(ctx, sysin.ServeLogListModel{}, &dao.SysServeLog, []*cgorm.Join{
