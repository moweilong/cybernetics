...
import (
	...
	"cybernetics/internal/library/casbin"
	"cybernetics/internal/library/cgorm"
...
func (s *sAdminRole) Edit(ctx context.Context, in *adminin.RoleEditInp) (err error) {
	if err = cgorm.IsUnique(ctx, &dao.AdminRole, g.Map{dao.AdminRole.Columns().Name: in.Name}, "名称已存在", in.Id); err != nil {
		return
	}

	if err = cgorm.IsUnique(ctx, &dao.AdminRole, g.Map{dao.AdminRole.Columns().Key: in.Key}, "编码已存在", in.Id); err != nil {
		return
	}

	if in.Pid, in.Level, in.Tree, err = cgorm.GenSubTree(ctx, &dao.AdminRole, in.Pid); err != nil {
		return
	}
